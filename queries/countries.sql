...
FROM
    world.countries c
        JOIN
    world.languages l ON c.id = l.country_id
WHERE
    l.language = 'Slovene'
...
SELECT 
    c.name, COUNT(ct.name) cities
FROM
    world.countries c
        JOIN
    world.cities ct ON c.id = ct.country_id
GROUP BY c.name
...
SELECT 
    ct.name, ct.population, ct.country_id
FROM
    world.countries c
        JOIN
    world.cities ct ON c.id = ct.country_id
...
SELECT 
    c.name, c.surface_area, c.population
FROM
    world.countries c
        JOIN
    world.cities ct ON c.id = ct.country_id
...
SELECT 
    c.name, c.government_form, c.capital, c.life_expectancy
FROM
    world.countries c
WHERE
    c.government_form = 'Constitutional Monarchy'
        AND c.capital > 200
...
SELECT 
    c.name, ct.name, ct.district, ct.population
FROM
    world.countries c
        JOIN
    world.cities ct ON c.id = ct.country_id
...
SELECT 
    c.region, COUNT(c.name) AS countries
FROM
    world.countries c
GROUP BY c.region
